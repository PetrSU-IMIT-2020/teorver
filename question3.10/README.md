...
$f_\xi(x|_y)$ (в формулах ниже также используется это обозначение, но latex'у плохо, поэтому 
там просто $x_y$ ) действительной 
переменной $x$, определяемая 
при всех $x \in R$ следующей формулой умножений 
для плотностей: 

$$\displaystyle f_\xi(x_y) = \frac{f_{\xi, \eta}(x, y)}{f_\eta(y)}$$

Аналогично, при всех $y \in R$ и всех
$x \in R$ таких, 
что $f_\xi(x) > 0$

$$\displaystyle f_\eta(y_x) = \frac{f_{\xi, \eta}(x, y)}{f_\xi(x)}$$
